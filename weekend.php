            ...
            <div id="weekend_head">Weekends
            <div class="create_click">Create Weekend</div>
            </div>
            
        <script>
            //display weekend location on the map     
            function weekend_map(location,display) {
                ...
                var img_url="http://maps.googleapis.com/maps/api/staticmap?center="+location+"&zoom=14&size=370x240&sensor=true";
             ...
             $places=getSuburbs($place);
             
             //TROUBLE SHOOTING
             //echo '<pre>',print_r($my_friends),'</pre>';
...
?>
        <!--/scripts to get each id for weekend location get the name and call function to display on a google map-->
<script>
                var index='<?php echo $index; ?>';
            var wk_loc=document.getElementsByClassName("wk_loc");
            var show_loc_map=document.getElementsByClassName("show_loc_map");  
                                //TROUBLESHOOTING
                                for (var i=0;i<wk_loc.length;i++){
                                    
                                     console.log( "I have collected " + wk_loc[i].innerHTML + " From class");
                                     var loc_node=wk_loc[i].innerHTML;
                                     var dis_node=show_loc_map[i];
                                      console.log(dis_node);
                                       weekend_map(loc_node,dis_node);    
                                }
            
            </script>

    <!-----------------------------THIS IS AJAX SOLUTION-------------------------------------------------> 
    
    <!--ajax call to bring API display here-->
    <script>
        //CREATE AJAX CALL function to get the respose from the server
        var xhr;
        function callSource(){
            xhr= new XMLHttpRequest();
            xhr.onreadystatechange=callback;
            xhr.open("GET","api/weekend_api.php",true);
            xhr.send();
        }    
    ...
    <!--display aajax result here-->
    <div id="result"></div>
    
    <!--Create on click event to call eventlistener function to run API call-->
    <!--<input type="button" value="ajax call" onclick="callSource()"/>-->
